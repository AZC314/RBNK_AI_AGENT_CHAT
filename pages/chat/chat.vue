<template>
	<view class="chat-container">
		<z-paging ref="zPagingRef" v-model="msgList" :scroll-to="scrollTo" @query="loadMoreMessages"
			@refresh="onRefresh" :auto="false" :auto-show-back-to-top="false" :to-top-disabled="true"
			:show-loading-more-when-no-more="true" :refresher-enabled="true" :refresher-threshold="80"
			:refresher-default-style="'black'" :refresher-background="'#FFF'" :loading-more-enabled="false"
			:empty-view-text="''" :auto-scroll-to-top-when-reload="false" :show-empty-view="false"
			...
			<view v-for="msg in msgList" :key="msg.msg.id">
				...
				<kit-chat-other-msg v-else :msg="msg.msg" @click="handleMessageClick" @feedback="handleFeedback"
					@like="handleLike" @dislike="handleDislike" @undislike="handleUndislike" @unlike="handleUnlike" />
			</view>
...
<script lang="ts" setup>
	import {
		Ref, ref, onMounted, onBeforeUnmount, watch, provide, reactive, nextTick
	} from 'vue'
	...
	const handleImageUpload = async (file : File) => {
		try {
			uni.showToast({ title: '图片上传功能开发中', icon: 'none', duration: 2000 });
		} catch (error) {
	...
	const handleAttachmentUpload = async (file : File) => {
		try {
			uni.showToast({ title: '附件上传功能开发中', icon: 'none', duration: 2000 });
		} catch (error) {
			uni.showToast({ title: '附件上传失败', icon: 'error', duration: 2000 });
		}
	};
	...
	const handleVoiceRecord = async (audioBlob : Blob) => {
		try {
			uni.showToast({ title: '语音录制功能开发中', icon: 'none', duration: 2000 });
		} catch (error) {
			uni.showToast({ title: '语音录制失败', icon: 'error', duration: 2000 });
		}
	};
	...
	const handleClear = () => {
		try {
			if (msgList.value.length < 1) return;
			uni.showModal({
				title: '清除对话消息',
	...
	const handleToggleSettings = () => {
		try {
			...
			uni.showToast({ title: '切换设置失败', icon: 'error', duration: 2000 });
		}
	};
				...
				try {
					...
					if (successfully) {
						...
						if (messageResponse.length === 1) {
							await messageStore.addMessage(chatId, messageResponse[0]);
						}
					}
				} catch (error) {
					uni.showToast({
	...
	const handleMessageClick = (type : string, content : any) => {
		switch (type) {
			...
			case 'file':
				...
				break;
			case 'video':
				uni.navigateTo({ url: `/pages/video-player/video-player?url=${encodeURIComponent(content.videoUrl)}` });
				break;
			default:
				// 其他类型
				break;
	...
	async function loadAvatar() {
		avatar.value = await avatarStore.getAvatarUrl(chatId);
	}
	...
	async function init() {
		try {
			...
			const allLocalMessages = await messageStore.getMessages(chatId); // 必须await
			// 只筛选当前会话conversationId的消息
			const localMessages = allLocalMessages.filter(msg => msg.conversation_id === conversationId);

			// 获取最近一次本地加载时间
			const lastLoadedAt = messageStore.getLastLoadedAt(chatId);
			const now = Date.now();

			let needFetchRemote = false;
			if (localMessages.length === 0) {
				needFetchRemote = true;
			} else if (now - lastLoadedAt > 3 * 60 * 1000) {
				needFetchRemote = true;
			}

			msgList.value = [];

			if (needFetchRemote && conversationId) {
				// 4. 拉取远程消息
				const res = await GET_MESSAGE({ conversation_id: conversationId, limit: 10 });
				const data = res as Info.Message[];
				await messageStore.deleteMessages(chatId); // 清空本地
				for (const elem of data) {
					await messageStore.addMessage(chatId, elem);
				}
				data.forEach(elem => {
				...
				console.log('拉取远程消息加载消息列表成功');
				console.log(data);
			} else {
				// 5. 用本地消息
				localMessages.forEach(elem => {
					const format = ChatMessage.ChatHistory2ChatMessage(elem, 'text', { agentId: chatId, username: chatTitle.value });
					msgList.value.push(...format);
				});
				console.log('用本地消息加载消息列表成功');
	...
	const onRefresh = async () => {
		try {
			...
			// 获取当前会话的消息列表
			const currentMessages = await messageStore.getMessages(chatId) || []
			const firstMsg = currentMessages[0]
			const first_id = firstMsg ? firstMsg.message_id || firstMsg.id : undefined
			const params : any = { conversation_id: conversationId, limit: 10 }
			...
			if (data && data.length > 0) {
				...
				const newMessages = (await Promise.all(data.map(msg => ChatMessage.ChatHistory2ChatMessage(msg, 'text', { agentId: chatId, username: chatTitle.value })))).flat()
				msgList.value = [...newMessages, ...msgList.value]
				console.log('更新消息成功，最新的消息列表\n'+ msgList.value);
				zPagingRef.value.complete(msgList.value)
			} else {
